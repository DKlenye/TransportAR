...
namespace Transport.Models
{
   ...
   [Model,ActiveRecord]
    public class WaybillTask:ActiveRecordBase<WaybillTask>
    {
      ...
      public WaybillTask() {
        ...
        [AllowBlank, HasMany(Table = "WaybillTaskIncrease",ColumnKey="TaskId",Inverse=true,Cascade=ManyRelationCascadeEnum.AllDeleteOrphan,Fetch=FetchEnum.SubSelect)]
        ...
        public void CheckWinter(){
           ...
           var targetDate = new DateTime(TaskDepartureDate.Year, TaskDepartureDate.Month, TaskDepartureDate.Day, DepartureDate.Hour, DepartureDate.Minute, 0);
           ...
           if( NormConsumptionId != null && FuelId != null && WorkAmount !=null ) {
              ...
              if(isTruck !=null && isTruck.Value){
                     ...
                     if(TrailerId!= null){
                        ...
                        CONSUMPTION = this.calculateTruckWithTrailerConsumption(consumption, trailerWeight, Qt, WorkAmount ?? 0, WeightKm ?? 0, Qv, Weight ?? 0, tkmConsumption, increaseK);
                        return CONSUMPTION;
                     }


                     CONSUMPTION = this.calculateTruckConsumption(consumption, ( WorkAmount ?? 0 ), Qv, ( Weight ?? 0 ), ( WeightKm ?? 0 ), tkmConsumption, increaseK);
                     return CONSUMPTION;
                 ...
                 if( TrailerId != null ) {
                    ...
                    trailerConsumption = ( WorkAmount ?? 0 ) * (TrailerWeight ?? 0) * ( tkmConsumption / (decimal)100.0 ) * increaseKWeight;
      ...
      public decimal calculateTruckConsumption(decimal H, decimal L, decimal q, decimal m, decimal S, decimal c, decimal increaseK){
      ...
      public decimal calculateTruckWithTrailerConsumption(decimal H, decimal Mt, decimal Qt, decimal L, decimal S, decimal q, decimal m, decimal c, decimal increaseK)
      {                              
     ...
     public decimal calculateTruckTrailerConsumption ( decimal H, decimal Mt, decimal Qt, decimal L, decimal S, decimal q, decimal m, decimal c, decimal increaseK){
   ...
   public class WaybillTaskIncrease : ActiveRecordBase<WaybillTaskIncrease> {
      ...
      public int TaskId { get { return key.TaskId; } set { if (key == null) key = new WaybillTaskIncreaseKey(); key.TaskId = value; } }
      [IdProperty]
      public int IncreaseId { get { return key.IncreaseId; } set { if (key == null) key = new WaybillTaskIncreaseKey(); key.IncreaseId = value; } }
