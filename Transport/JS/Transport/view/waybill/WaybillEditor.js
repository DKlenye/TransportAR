T.view.waybill.WaybillEditor = Ext.extend(Kdn.editor.ModelEditor, {
    ...
    constructor: function(cfg) {
        ...
        var api = {
            read: Kdn.Direct.customerWorkingTimeRead,
            update: Kdn.Direct.Update
        };

        var customerWorkingTimeStore = new Kdn.data.DirectStore(
        ...
        cfg.closeWindow = new Ext.Window({
            ...
            items: [
                ...
                },
                {
                    ...
                    store: customerWorkingTimeStore
                },
                {
                    xtype: 'container',
                    height: 20
...
//функция контролирует список остатков по доступным транспорту видам топлива
    ...
    onAfterCheckDatePeriod: function(e) {
        if (e == 0) {
            ...
            this.closeWindow.workingtime.store.load({
                params: {
                    WaybillId: this.waybillId
                }
            });
