...
//  MOUSE NAVIGATION
//  *****************************************************************************
function setMouseNavigation(Cooka,AGrid, ABgColor,AFgColor , ABgSelect,AFgSelect, AArrayKeyCols) {
 ...
 AGrid.ex__bgColor  = "pink"; 
 AGrid.ex__fgColor  =  "black"; 
 AGrid.ex__bgSelect = "PaleVioletRed";
 AGrid.ex__fgSelect =  "black"; 
  ...
  if (AArrayKeyCols == null) {
   ...
   mouseNavigationOut(AGrid.rows(i));
   }
...
function mouseNavigationClick(AObjEvent) {
  ...
  if (obj.tagName == "TD") {
    ...
    if (obj.offsetParent.selectedRow) {
      ...
      if (obj.offsetParent.className='Act') mouseNavigationSetColor(obj.offsetParent.selectedRow, obj.ex__bgColor, obj.ex__fgColor, true);
      else
    ...
    if (!isActive || !obj.offsetParent.allowNullSelection) {
     ...
     if(obj.parentElement.className=='Act')
      mouseNavigationSetColor(obj.parentElement,'darkmagenta', 'white');
 else    mouseNavigationSetColor(obj.parentElement, obj.offsetParent.__bgSelect, obj.offsetParent.__fgSelect);
...
//
function mouseNavigationOver(objc) {

if (objc) var obj=objc
else{
  event.cancelBubble = true;
  var obj = event.srcElement;
  }
  if (obj.parentElement.className=='Act')
   mouseNavigationSetColor(obj.parentElement, obj.offsetParent.ex__bgColor, obj.offsetParent.ex__fgColor);
   else
...
//
function mouseNavigationOut(objc) {
 if (objc) 
 {
 var obj=objc;
 
  if (obj.className=='Act')
   mouseNavigationSetColor(obj, obj.offsetParent.ex__bgSelect, obj.offsetParent.ex__fgSelect);
   else
  mouseNavigationSetColor(obj, null, null);
 ...
 // alert(obj.parentElement.className);
   if (obj.parentElement.className=='Act')
   mouseNavigationSetColor(obj.parentElement, obj.offsetParent.ex__bgSelect, obj.offsetParent.ex__fgSelect);
   else
  mouseNavigationSetColor(obj.parentElement, null, null);
}
//
...
function Loaded() {
  ...
  for (var i=0; i<dGrid.rows.item(0).cells.length; i++) {
    ...
    hTd.style.display = dGrid.rows.item(0).cells.item(i).style.display;
  ...
  setMouseNavigation(true,dGrid, "E9E9E9","BUTTONTEXT" , "HIGHLIGHT","HIGHLIGHTTEXT");
