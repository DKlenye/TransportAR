...
namespace Ext.Direct
{
    internal static class Utility
    {
        ...
        {
            return (T)attributeProvider.GetCustomAttributes(typeof(T),(true)).SingleOrDefault();
        ...
        {
            return GetName(t.GetCustomAttributes(typeof(DirectAliasAttribute), true), t.Name);
        }
        ...
        {
            return GetName(mi.GetCustomAttributes(typeof(DirectAliasAttribute), true), mi.Name);
        ...
        private static string GetName(object[] attrs, string defaultValue)
        {
            if (attrs.Length > 0)
            {
                var attr = attrs[0] as DirectAliasAttribute;
                if (attr != null)
                {
